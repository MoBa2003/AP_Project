...
namespace EssentialClasses
{
    public static class ValidationClass
    {
        ...
        public static void IDValidatate(this string id)
        {
            Regex regex = new Regex("^00[0-9]{8}$");
            if (!regex.IsMatch(id)) throw new Exception("Invalid SSN!");
            foreach (var cust in Customer.customers) if (cust.ID == id) throw new Exception("This SSN is already in use!");

        }
        public static void SSNValidate(this string id)
        {
            Regex regex = new Regex("^[0-9]{2}9[0-9]{2}$");
            if (!regex.IsMatch(id)) throw new Exception("Invalid ID!");
            foreach (var emp in Employee.employees) if (emp.SSN == id) throw new Exception("This ID is already in use!");
